...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@borg.on(d3vil_cmd(pattern="gpromote ?(.*)"))
...
@borg.on(d3vil_cmd(pattern="gdemote ?(.*)"))
async def gben(userbot):
    dc = dark = userbot
    i = 0
    sender = await dc.get_sender()
...
CmdHelp("global promote").add_command(
  "gpromote", "<reply>/<userid>", "Globally Promote the mentioned user in 'X' chats you are admin with ban permission."
).add_command(
  "gdemote", "<reply>/<userid>", "Globally Demote the user in 'X' chats you are admin!"
).add()
