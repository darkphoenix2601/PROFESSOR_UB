...
@borg.on(d3vil_cmd(pattern=("fakemail ?(.*)")))
async def _(event):
   if event.fwd_from:
      return 
   chat = "@fakemailbot"
...
@borg.on(d3vil_cmd(pattern=("mailid ?(.*)")))
async def _(event):
   if event.fwd_from:
      return 
   chat = "@fakemailbot"
...
CmdHelp("fake_mail").add_command(
  'sg', '<reply> <text>', 'Use this cmnd to know user name/username history'
).add_command(
  'gid', '<reply> <text>', 'Get Use id of user'
).add_command(
  'ub', '<reply> <text>'
).add()
