...
async def convert_to_image(event, bot):
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if d3vil and d3vil.photo:
        d3vil_final = downloaded_file_name
    elif d3vil.sticker and d3vil.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        d3vil_final = image_name20
    elif d3vil.sticker and d3vil.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        d3vil_final = imgpath
    elif d3vil.gif or d3vil.video or d3vil.video_note:
...
def tgs_to_gif(sticker_path: str, quality: int = 256) -> str:                  
    semx = os.path.join(dwlpath, "d3vilbottgs.gif")
